...
output_file = f'{root}/Code/Model_FJK/Output_Logs/agg_model_output_{current_time}.txt'
...
from sklearn.model_selection import RandomizedSearchCV, GridSearchCV,GroupKFold, KFold, train_test_split
from sklearn.ensemble import RandomForestRegressor
...
##### Import data
datasets = {
    'from_2000': pd.read_csv(f"{root}/Data/FINAL_FOR_MODEL/FJK_Agg_Sets/From_2000.csv", usecols=lambda column: column != 'Unnamed: 0'),
    'minus_3': pd.read_csv(f"{root}/Data/FINAL_FOR_MODEL/FJK_Agg_Sets/minus_3_long.csv", usecols=lambda column: column != 'Unnamed: 0'),
    'minus_5': pd.read_csv(f"{root}/Data/FINAL_FOR_MODEL/FJK_Agg_Sets/minus_5_long.csv", usecols=lambda column: column != 'Unnamed: 0'),
    'minus_10': pd.read_csv(f"{root}/Data/FINAL_FOR_MODEL/FJK_Agg_Sets/minus_10_long.csv", usecols=lambda column: column != 'Unnamed: 0')
}
...
for dataset_name, data in datasets.items():
    ...
    for response_col in response_variables:
        ...
        baseline_mae_mean = mean_absolute_error(y_test, baseline_pred)
        ...
        print(f"    R²: {r2: 5f}")
        print(f"    MAE : {mae: 5f}")
        print(f"    Baseline MAE (median prediction): {baseline_mae: 5f}")
        print(f"    Baseline MAE (mean prediction): {baseline_mae_mean: 5f}")
...
param_results.to_csv(f"{root}/Data/FINAL_FOR_MODEL/FJK_Agg_Results/param_results.csv")
mae_results.to_csv(f"{root}/Data/FINAL_FOR_MODEL/FJK_Agg_Results/mae_results.csv")
mse_results.to_csv(f"{root}/Data/FINAL_FOR_MODEL/FJK_Agg_Results/mse_results.csv")
rmse_results.to_csv(f"{root}/Data/FINAL_FOR_MODEL/FJK_Agg_Results/rmse_results.csv")
baseline_mae_results.to_csv(f"{root}/Data/FINAL_FOR_MODEL/FJK_Agg_Results/baseline_mae_results.csv")

r2_results.to_csv(f"{root}/Data/FINAL_FOR_MODEL/FJK_Agg_Results/r2_results.csv")
importance_results.to_csv(f"{root}/Data/FINAL_FOR_MODEL/FJK_Agg_Results/mean_importance.csv")
